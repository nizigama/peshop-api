...
class AdminUserService
{
    ...
    public function listNormalUsers(ListUsersRequestDTO $dto): Builder
    {
        ...
        return DB::table('users')
            ...
            ->when(!is_null($dto->first_name), function (Builder $query) use ($dto): void {
                $query->where('first_name', $dto->first_name);
            })
            ->when(!is_null($dto->email), function (Builder $query) use ($dto): void {
                $query->where('email', $dto->email);
            })
            ->when(!is_null($dto->phone), function (Builder $query) use ($dto): void {
                $query->where('phone_number', $dto->phone);
            })
            ->when(!is_null($dto->address), function (Builder $query) use ($dto): void {
                $query->where('address', $dto->address);
            })
            ->when(!is_null($dto->created_at), function (Builder $query) use ($dto): void {
                $query->where('created_at', Carbon::parse($dto->created_at));
            })
            ->when($dto->marketing, function (Builder $query) use ($dto): void {
